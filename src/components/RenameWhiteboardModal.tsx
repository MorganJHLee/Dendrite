...
interface RenameWhiteboardModalProps {
  currentName: string
  onRenameWhiteboard: (whiteboardName: string) => void
  onClose: () => void
}

export default function RenameWhiteboardModal({ currentName, onRenameWhiteboard, onClose }: RenameWhiteboardModalProps) {
  const [whiteboardName, setWhiteboardName] = useState(currentName)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (whiteboardName.trim() && whiteboardName.trim() !== currentName) {
      onRenameWhiteboard(whiteboardName.trim())
    }
    onClose()
  }
